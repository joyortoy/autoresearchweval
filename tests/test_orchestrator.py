...
class OrchestratorTests(unittest.TestCase):
    def test_dry_run_works(self):
        cp = subprocess.run(["python3.11", "-m", "autoresearch.orchestrator", "--dry-run", "--run-name", "t1"], cwd=ROOT, text=True, capture_output=True)
    ...
    def test_trust_failure_fails_closed(self):
        ...
        cp = subprocess.run(["python3.11", "-m", "autoresearch.orchestrator", "--dry-run", "--policy", str(bad_policy), "--run-name", "t2"], cwd=ROOT, text=True, capture_output=True)
        self.assertEqual(cp.returncode, 0)
        self.assertIn("discard", cp.stdout)

    def test_optional_evals_default_off(self):
        cp = subprocess.run(["python3.11", "-m", "autoresearch.orchestrator", "--dry-run", "--run-name", "t3"], cwd=ROOT, text=True, capture_output=True)
        self.assertEqual(cp.returncode, 0)
        payload = json.loads(cp.stdout)
